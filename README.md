...
Checkpoint 5 adds distributed trace lookup and basic root-cause detection. A trace can be fetched by `correlationId` in timestamp-ascending order, and the root-cause endpoint identifies the first serious failure signal in the trace.
...
Checkpoint 11 closes the architecture and LLD completion gaps. Logs are enriched with message hashes, exception fingerprints, severity scores, ingestion delay, and error flags; PII-sensitive values are masked before persistence; malformed Kafka events are persisted to `dead_letter_logs`; trace APIs include richer summaries, filters, transaction lookup, and user lookup; alert summaries can be generated by alert ID; rate limiting and payload limits are configurable; demo trade transaction endpoints and the reusable Java SDK package are now present.
...
GET  http://localhost:8080/api/v1/traces/{correlationId}?level=ERROR&serviceName=limit-check-service&includeStackTrace=true
GET  http://localhost:8080/api/v1/traces/by-transaction/{transactionId}
GET  http://localhost:8080/api/v1/traces/by-user/{userId}?limit=10
...
GET  http://localhost:8080/api/v1/incidents/{alertId}/summary
...
GET  http://localhost:8080/api/v1/analytics/error-rate?serviceName=trade-service&window=10m
GET  http://localhost:8080/api/v1/analytics/top-exceptions?window=10m&limit=5
POST http://localhost:8080/api/v1/demo/trade-transactions/success
POST http://localhost:8080/api/v1/demo/trade-transactions/fail-limit
```
...
curl "http://localhost:8080/api/v1/traces/corr-12345?level=ERROR&serviceName=limit-check-service&includeStackTrace=true"
curl "http://localhost:8080/api/v1/traces/by-transaction/TF-9081"
curl "http://localhost:8080/api/v1/traces/by-user/U1001?limit=5"
...
The trace response includes `status`, `durationMs`, `highestSeverity`, `services`, `firstEventTime`, `lastEventTime`, and ordered `events`. Stack traces are hidden by default and returned only when `includeStackTrace=true`.

The root-cause response prioritizes serious failures, returning the selected service, message, exception type, transaction/user identifiers, a `HIGH`, `MEDIUM`, or `LOW` confidence tier, a reason, and supporting timeline events around the failure. For the sample trace, it identifies `limit-check-service` with message `Customer limit validation failed`, exception type `LimitExceededException`, and confidence `HIGH`.
...
curl "http://localhost:8080/api/v1/incidents/{alertId}/summary"
curl "http://localhost:8080/api/v1/analytics/top-errors?window=10m&limit=5"
curl "http://localhost:8080/api/v1/analytics/error-rate?serviceName=trade-service&window=10m"
curl "http://localhost:8080/api/v1/analytics/top-exceptions?window=10m&limit=5"
```

Demo trade transaction flow:

```bash
curl -X POST "http://localhost:8080/api/v1/demo/trade-transactions/success"
curl -X POST "http://localhost:8080/api/v1/demo/trade-transactions/fail-limit"
```
...
The migration-managed schema now includes `logs`, `alerts`, `incident_summaries`, `dead_letter_logs`, and `service_registry`. Log writes are idempotent by `event_id`, and enrichment indexes cover message hash, exception fingerprint, severity/time, correlation, trace, user, transaction, service/time, and level/time lookup paths.
...
```json
{
  ...
  "exceptionType": "LimitExceededException",
  "confidence": "HIGH",
  "reason": "First prioritized ERROR event with exception type in the distributed request timeline"
}
```
